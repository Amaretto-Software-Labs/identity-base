...
import { getOrganisationRolePermissions, updateOrganisationRolePermissions } from '../api/organisations'
import type { OrganisationRole, OrganisationRolePermissions } from '../api/types'
...
export function OrganisationRolePermissionsPanel({ organisationId, roles, onStatusMessage }: RolePermissionsPanelProps) {
  const [activeRoleId, setActiveRoleId] = useState<string | null>(null)
  const [permissions, setPermissions] = useState<OrganisationRolePermissions | null>(null)
  ...
  useEffect(() => {
    if (!organisationId || !activeRoleId) {
      setPermissions(null)
      setEditablePermissions([])
      return
    }
    ...
    getOrganisationRolePermissions(organisationId, activeRoleId)
      .then((response) => {
        if (cancelled) return
  ...
  const handleSave = async () => {
    ...
    try {
      await updateOrganisationRolePermissions(organisationId, activeRoleId, payload)
      const updated = await getOrganisationRolePermissions(organisationId, activeRoleId)
    ...
    <section className="space-y-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.5fr,2fr]">
        <div className="space-y-2">
          {roles.length === 0 ? (
            <p className="text-sm text-slate-600">No organisation roles have been defined.</p>
          ) : (
            roles.map((role) => {
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    These permissions are applied specifically to this organisation. Permissions inherited from default role

...
import { useOrganisationMembers } from '@identity-base/react-organisations'
import { useVirtualizer } from '@tanstack/react-virtual'
import { listInvitations, getOrganisation, getOrganisationRoles } from '../api/organisations'
import type { InvitationResponse, OrganisationRole } from '../api/types'
import { renderApiError } from '../api/client'
import { OrganisationMemberRow } from '../components/OrganisationMemberRow'
import { OrganisationRolePermissionsPanel } from '../components/OrganisationRolePermissionsPanel'
import { OrganisationInvitationsPanel } from '../components/OrganisationInvitationsPanel'
...
export default function OrganisationAdminPage() {
  ...
  const {
    ...
    isPageLoaded,
    getMemberAt,
  } = useOrganisationMembers(organisationId, {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setLoadError(null)
      try {
        const [organisation, roleList, inviteList] = await Promise.all([
          getOrganisation(organisationId),
          getOrganisationRoles(organisationId).catch(() => []),
          listInvitations(organisationId).catch(() => []),
        ])
        ...
        setOrganisationName(organisation.displayName)
  ...
  useEffect(() => {
    if (!organisationId || memberTotalCount === 0) {
      return
    }

    ensurePage(currentPage).catch(() => undefined)
  }, [organisationId, currentPage, ensurePage, memberTotalCount])
  ...
  const handleRemoveMember = async (memberId: string) => {
    ...
    if (!window.confirm('Remove this member from the organisation?')) {
      return
    }
    ...
    try {
      ...
      setStatusMessage('Member removed from organisation.')
    } catch (err) {
      setMemberActionError(renderApiError(err))
    } finally {
      setMutatingMemberId(null)
  ...
  return (
    <div className="space-y-6">
      <header className="space-y-1">
        ...
        <h1 className="text-2xl font-semibold text-slate-900">{organisationName || 'Organisation'}</h1>
        <p className="text-sm text-slate-600">
          Manage memberships, inspect organisation roles, manage invitations, and configure role permissions. Permissions and scope enforcement are handled by the sample API using <code>RequireOrganisationPermission</code>.
        </p>
        <p className="text-xs text-slate-500">
          Slug: <span className="font-mono">{organisationSlug || 'unknown'}</span>
        </p>
      </header>
      ...
      {loadError && (
        <div className="rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700">
          Failed to load organisation. {loadError}
        </div>
      )}
      {membersError ? (
        <div className="rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700">
      ...
      {isLoading ? (
        <p className="text-sm text-slate-600">Loading organisation details…</p>
      ) : (
        <>
          {organisationId && (
            <OrganisationRolePermissionsPanel
                        ...
                        return (
                          ...
                          >
                            {member ? (
                              ...
                              <OrganisationMemberPlaceholderRow />
                            )}
                          </div>
                        )
                      })}
...
function OrganisationMemberPlaceholderRow() {
  return (

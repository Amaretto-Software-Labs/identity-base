...
builder.Services.Configure<OrganizationBootstrapOptions>(builder.Configuration.GetSection("SampleData:DefaultOrganization"));

builder.Services.AddScoped<OrganizationBootstrapService>();
builder.Services.AddScoped<IUserCreationListener, OrganizationBootstrapUserCreationListener>();

builder.Services.AddDbContext<OrgSampleDbContext>((provider, options) =>
{
    var configuration = provider.GetRequiredService<IConfiguration>();
    var connectionString = GetPrimaryConnectionString(configuration);
    options.UseNpgsql(connectionString, sql => sql.EnableRetryOnFailure());
});

builder.Services.AddScoped<IInvitationStore, EfInvitationStore>();
...
var organizationsBuilder = builder.Services.AddIdentityBaseOrganizations();
...
identityBuilder.AfterOrganizationSeed(async (sp, ct) =>
{
    ...
    var defaults = services.GetRequiredService<IOptions<OrganizationBootstrapOptions>>().Value;
    ...
    var bootstrapService = services.GetRequiredService<OrganizationBootstrapService>();
    ...
    await bootstrapService.EnsureOrganizationOwnerAsync(user, request, ct).ConfigureAwait(false);
});

var app = builder.Build();

await EnsureSampleDatabaseCreatedAsync(app.Services);
...
static string GetPrimaryConnectionString(IConfiguration configuration)
{
...
static async Task<IResult?> EnsureActorInScopeAsync(
    ClaimsPrincipal principal,
    IOrganizationScopeResolver scopeResolver,
    Guid organizationId,
    CancellationToken cancellationToken)
...
static async Task EnsureSampleDatabaseCreatedAsync(IServiceProvider services)
{
    ArgumentNullException.ThrowIfNull(services);

    await using var scope = services.CreateAsyncScope();
    var dbContext = scope.ServiceProvider.GetRequiredService<OrgSampleDbContext>();

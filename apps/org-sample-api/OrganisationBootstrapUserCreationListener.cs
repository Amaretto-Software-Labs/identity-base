...
public sealed class OrganisationBootstrapUserCreationListener : IUserCreationListener
{
    private static readonly string[] NameKeys = { "organisationName", "organisation.name", "organisationDisplayName" };
    private static readonly string[] SlugKeys = { "organisationSlug", "organisation.slug" };
    private const string MetadataPrefix = "organisation.metadata.";

    private readonly OrganisationBootstrapService _bootstrapService;
    private readonly IOptions<OrganisationBootstrapOptions> _defaults;
    private readonly IOptions<IdentitySeedOptions> _seedOptions;
    private readonly ILogger<OrganisationBootstrapUserCreationListener> _logger;
    private readonly IHttpContextAccessor _httpContextAccessor;

    public OrganisationBootstrapUserCreationListener(
        OrganisationBootstrapService bootstrapService,
        IOptions<OrganisationBootstrapOptions> defaults,
        IOptions<IdentitySeedOptions> seedOptions,
        ILogger<OrganisationBootstrapUserCreationListener> logger,
    ...
    public async Task OnUserCreatedAsync(ApplicationUser user, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogDebug("Skipping organisation bootstrap for user {UserId} because an invitation code is present.", user.Id);
            return;
        }
        ...
        if (request is null)
        {
            _logger.LogDebug("No organisation bootstrap metadata supplied for user {UserId}.", user.Id);
            return;
        }

        try
        {
            await _bootstrapService.EnsureOrganisationOwnerAsync(user, request, cancellationToken).ConfigureAwait(false);
        }
        catch (Exception exception) when (!cancellationToken.IsCancellationRequested)
        {
            _logger.LogError(
                exception,
                "Failed to ensure organisation membership for user {UserId} ({Email}).",
                user.Id,
                user.Email);
        }
    }

    private static OrganisationBootstrapRequest? ResolveFromMetadata(UserProfileMetadata metadata)
    {
        if (metadata is null)
        {
            return null;
        ...
        return new OrganisationBootstrapRequest(name.Trim(), slug, metadataValues);
    }
    ...
    private OrganisationBootstrapRequest? ResolveFromDefaults()
    {
        ...
        return new OrganisationBootstrapRequest(
            name,
            defaults.Slug,
            metadata);
    }

...
3. **User Flows** – The user endpoint (`GET /users/me/organizations`) exposes memberships. Clients select an active organization purely by sending the `X-Organization-Id` header; there is no server-side “set active organization” endpoint.

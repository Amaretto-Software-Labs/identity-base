...
| `Identity.Base.ServicePrincipals` | Managed machine identities with independently revocable credentials, RBAC role assignments, OpenIddict client-credentials integration, and admin APIs. | `Identity.Base`, `Identity.Base.Roles`, `Identity.Base.Admin` | Optional | `services.AddIdentityBaseServicePrincipals(...)` and `app.MapIdentityBaseServicePrincipalEndpoints()` |

Packages compose in layers: run only the core, add RBAC when you need permissions, layer organizations for multi-org apps, and opt into admin or service-principal endpoints as needed. Service principals depend on the admin authorization helpers and shared RBAC store but remain a separately registered package. Nothing turns on automatically—service registration, DbContext configuration, migrations, middleware, and endpoint mapping are explicit.
...
- **Service Principals** – Stored separately from users and organizations. A principal owns multiple credential hashes and is linked to global RBAC roles through `ServicePrincipalRole`; it never acquires user membership or organization context implicitly.
...
6. **Service Principals** – Managed service principals retain an immutable `Guid` subject and generated client ID in a dedicated host-migrated context. Their credentials are one-time-returned, salted hashes at rest. The package replaces only OpenIddict's client-secret validation handler, delegates unmanaged clients back to the standard application manager, and emits `identity.principal_type=ServicePrincipal` plus role-derived `identity.permissions`. Tokens are short-lived and carry no refresh token. Disable and revoke-all revoke stored token entries; consumers that perform only offline JWT validation must account for the normal delay until token expiry.
7. **Client SDKs** – `@identity-base/client-core` owns PKCE, token storage/refresh, cookie fallback, normalized HTTP errors, and typed admin namespaces. React and Angular packages layer framework state, guards, interceptors, and organization workflows over the same contracts.
...
- Immediate service-principal token revocation depends on token-entry-aware validation. A downstream API that validates self-contained JWTs without consulting the authority cannot observe revocation before `exp`; keep managed token lifetimes short or add an online validation strategy for higher-risk workloads.

...
6. **Service Principals** – Managed service principals retain an immutable Guid subject and client ID in a dedicated host-migrated context. Their credentials are one-time-returned, salted hashes at rest. The package replaces only OpenIddict's client-secret validation handler, delegates unmanaged clients back to the standard application manager, and emits `identity.principal_type=ServicePrincipal` plus role-derived `identity.permissions`. Token-entry validation is enabled so disable/revoke-all can invalidate already-issued tokens.

...
- **Extensibility Hooks:** expose multi-tenant and portal behaviour by composing the public hooks (`ConfigureAppDbContextModel`, `ConfigureIdentityRolesModel`, `ConfigureOrganisationModel`, `AfterRoleSeeding`, `AfterIdentitySeed`, `AfterOrganisationSeed`, custom `IPermissionClaimFormatter`, `AddOrganisationClaimFormatter`, `IPermissionScopeResolver`, `AddOrganisationScopeResolver`) rather than modifying OSS code paths.
